...
async function test() {
    ...
    assertEq('quoteMint', merpsGroup.tokens[0].toBase58(), quoteMintKey.toBase58())
    assertEq('admin', merpsGroup.admin.toBase58(), payer.publicKey.toBase58())
    assertEq('dexProgramId', merpsGroup.dexProgramId.toBase58(), dexProgramId.toBase58())
    const cacheRootBanksTxID = await client.cacheRootBanks(payer, merpsGroup.publicKey, merpsGroup.merpsCache);
