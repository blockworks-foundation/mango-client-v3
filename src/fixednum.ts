...
export class I80F48 {
   ...
   */
  static MAX_SIZE: number = 128;
  static FRACTIONS: number = 48;
  ...
  constructor(data: BN) {
    this.maxValue = (new BN(2).pow(new BN(I80F48.MAX_SIZE)).div(new BN(2))).sub(new BN(1));
    this.minValue = (new BN(2).pow(new BN(I80F48.MAX_SIZE)).div(new BN(2))).neg();
    if (data.lt(this.minValue) || data.gt(this.maxValue)) {
      throw new Error("Number out of range");
    }
    this.data = data;
    this.binaryLayout = data.toTwos(I80F48.MAX_SIZE).toString(2, I80F48.MAX_SIZE).replace(/\-/g, '');
  }
  ...
  mul(x: I80F48): I80F48 {
    const divider = (new Big(2)).pow(I80F48.FRACTIONS);
    const result = new Big(this.data.mul(x.getInternalValue()).toString()).div(divider).round().toFixed();
    return new I80F48(new BN(result));
  }

  div(x: I80F48): I80F48 {
    const multiplier = (new Big(2)).pow(I80F48.FRACTIONS);
    const result = new Big(this.data.toString()).div(x.getInternalValue().toString()).times(multiplier).round().toFixed();
    return new I80F48(new BN(result));
  }
}
